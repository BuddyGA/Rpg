...
{
	static RpgFilePath ExecutableDirPath;
...
void RpgFileSystem::Initialize() noexcept
{
	...
	// Executable
	{
		tempPathString = SDL_GetBasePath();
		tempPathString.ReplaceInPlace('\\', '/');
		ExecutableDirPath = tempPathString;
	}
...
#else
	ProjectDirPath = ExecutableDirPath.GetParentDirectoryPath().GetParentDirectoryPath().ToString();
...
{
	return ExecutableDirPath.ToString();
}
